...
3. 保持终端窗口开启。
4. 在 iPhone Safari 打开终端显示的“推荐”完整网址。推荐地址使用局域网 IP（例如 `192.168.x.x`），添加到主屏幕时比 `.local` 地址更稳定。
5. 在 Safari 分享菜单中选择“添加到主屏幕”。

添加到主屏幕时，请从终端显示的完整 `?code=...` 地址执行添加。主屏幕版第一次打开时如果要求配对，输入终端中的六位配对码即可；配对码只保存在这台 iPhone 上。
